...
def actor_learner_thread(thread_id, env, session, graph_ops, num_actions,
                         summary_ops, saver, output, env_name):
    """
    ...
    while T < TMAX:
        ...
        while True:
            ...
            if t % checkpoint_interval == 0:
                saver.save(session, "/home/karan/Desktop/final_year_project/Pong-v0" , global_step=t) ###  output +"/"+ env_name
...
def train(session, graph_ops, num_actions, saver, output, threads, summary, env_name):
    """
    Train a model.
    """

    # Set up game environments (one per thread)
    envs = [gym.make(env_name) for i in range(threads)]
    ...
    actor_learner_threads = \
        [threading.Thread(target=actor_learner_thread,
                          args=(thread_id, envs[thread_id], session,
                                graph_ops, num_actions, summary_ops, saver, output, env_name))
...
def evaluation(session, graph_ops, saver, output, env_name, episodes, model):
    """
    Evaluate a model.
    """
    saver.restore(session, model)
    monitor_env = gym.make(env_name)
    monitor_env = gym.wrappers.Monitor(monitor_env, output + "/" + env_name, force=True)
    #monitor_env.monitor.start(output + "/" + env_name, force=True)
    ...
    for i_episode in xrange(episodes):
        print("Start episode ", i_episode+1)
        s_t = env.get_initial_state()
        ep_reward = 0
        terminal = False
        while not terminal:
...
def main(_):
    ...
    with tf.Session() as session:
        ...
        parser.add_argument("--mode", default="eval", required=True, 
                                      help="mode of script, available values are eval or train")
        parser.add_argument("--env", default="Pong-v0", help="environment name")
        parser.add_argument("--output", default="out", help="output directory for trained model")
        parser.add_argument("--threads", type=int, default=2, help="number of learning threads")
        parser.add_argument("--summary", default="logs", help="directory for logs")
        ...
        num_actions = get_num_actions(args.env);
        graph_ops = build_graph(num_actions)
        saver = tf.train.Saver(max_to_keep=5)

        if args.mode == "eval":
            evaluation(session, graph_ops, saver, 
                       args.output, args.env, args.episodes, args.model)
        elif args.mode == "train":
            train(session, graph_ops, num_actions, saver, 
                  args.output, args.threads, args.summary, args.env)
        else:
            pass    
